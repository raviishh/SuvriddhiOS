...
curl --max-time 30 -k -o /tmp/release.json https://api.github.com/repos/raviishh/SuvriddhiOS/releases/latest
DOWNLOAD_URL=$(grep -o '"browser_download_url": *"[^"]*"' /tmp/release.json | grep -o 'https://[^"]*')
curl --max-time 60 -L -k -o /tmp/suvriddhi.zip "$DOWNLOAD_URL"
