...
int32_t read(struct mg_connection *conn, std::vector<char> &buf)
{
	...
	while (true) {
		req_bytes = mg_read(conn, buf.data() + total_bytes, buf.size() - total_bytes);
		if (req_bytes < 0) {
			return -1; // Read error
		}
		total_bytes += req_bytes;
		if (total_bytes >= static_cast<int32_t>(kMaxRequestBytes)) {
			return -1; // Request is too large
		}
		if (total_bytes >= static_cast<int>(buf.size())) {
			buf.resize(std::min(buf.size() * 2, static_cast<size_t>(kMaxRequestBytes)));
		} else {
